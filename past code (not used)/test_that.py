...
output_pin = arduino.get_pin('d:9:p') #digital, PWM mode, from the computer
...
initial_5V = 5#(1000+220)/220*read_voltage(input5V_pin)
...
def measure_temperatures(DC):
    ...
    input5V_voltage = 5#(1000+220)/220*read_voltage(input5V_pin)
...
def record_temperature(temperature, DC, incoming_voltage, input5V_voltage):
    try:
        with open('_4203_project_4' + str(start_time) + '.csv', 'a', newline='') as csvfile:
            ...
            temperaturewriter.writerow([time.time() - start_time, DC, temperature, incoming_voltage, get_set_point_temperature(time.time()-start_time)])  
    except:
...
def run():
    with open('_4203_project_4' + str(start_time) + '.csv', 'a', newline='') as csvfile:
        temperaturewriter = csv.writer(csvfile, delimiter=',')
        temperaturewriter.writerow(['time', 'DC', 'temperature', 'incoming_voltage', 'set_point_temperature'])
    DC = 0.1
